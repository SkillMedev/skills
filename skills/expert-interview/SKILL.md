...
1. **Learning objectives**: what decision the interviews inform, and the 3-5 things that must be learned. Separate "must learn" from "nice to know" so sections can be cut under time pressure.
2. **The hypothesis being tested**, if any — written down so it can be deliberately kept out of the questions.
3. **Interview length** (default 45-60 minutes) and format (call, in person).
4. **How many experts** and what mix of vantage points (practitioner, buyer, former insider, adjacent-market operator).
5. **Recording and consent constraints**, and any compliance rules (expert networks prohibit asking for confidential or MNPI material — respect this in question design).
