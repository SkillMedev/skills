...
Sources, cheapest first: the grantee lists (Schedule I or Part XV) of funders who fund your closest peer organizations; ProPublica Nonprofit Explorer full-text search of 990s for your program keywords and county; the local community foundation's directory; state nonprofit association funder lists; board members' personal knowledge of local foundations. Aim for 30-50 raw names to yield 16-20 qualified prospects.

### Step 3: Run the 990-reading procedure on each plausible funder
...
6. Officers and trustees. Cross-check names against your board's network. A warm introduction changes the odds materially.
7. Repeat grantees across the two filings. Many repeats signal a relationship funder that renews; plan for multi-year cultivation, not one-shot asks.

### Step 4: Fit-score the prospect

Score five criteria 0-3 each, 15 points maximum:

- Mission alignment: your program matches the grants-paid list (not the website language).
- Grant-size fit: your ask falls within 50-150 percent of the funder's median grant. Outside that band, score 0-1.
- Geographic fit: the funder has recent grantees in your county or state.
- Grantee similarity: at least one recent grantee resembles you in budget and program.
- Accessibility: accepts unsolicited LOIs, or you hold a genuine board connection.

Thresholds: 11-15 pursue, 8-10 watch (revisit when something changes), 7 or below disqualify.
...
Sort pursue-tier prospects by deadline, assign each an LOI month within staff capacity, and hand the top prospect to grant-loi-writer. Report pipeline coverage (total qualified asks divided by goal) to the board through nonprofit-board-pack.

## Inputs to collect

Annual budget, grant-revenue goal, program area and population served, geography served, minimum useful ask and typical ask, current and past funders, board members' foundation connections, and staff capacity for applications per month (default 1-2 LOIs for a 3-person team). If any number is a guess, label it a guess in the prospect table and refine later.
...
Riverton Community Foundation    3     2    3     3      0     11→9 WATCH
  Strong fit but grants by trustee nomination only; no connection yet.
  Action: ask board for introductions to Riverton trustees.
```

## Deliverable

A scored prospect table like the one above covering every raw-list name, pipeline coverage math (qualified asks vs 4-5x goal), a one-line reason for every disqualification, and a submission calendar matched to staff capacity, with the first prospect handed to grant-loi-writer.
...
- Every pursue-tier prospect has a 990-verified median grant and at least one named similar grantee.
- Pipeline coverage is computed and shown, not asserted, and sits at 4-5x the goal.
- Every disqualified funder carries a one-line reason.
- The submission calendar fits inside stated staff capacity.

## Escalation

When a prospect scores pursue, go to grant-loi-writer. When the pipeline table is ready for the board, feed the coverage numbers into nonprofit-board-pack. For government grants with compliance regimes (federal RFPs, single-audit thresholds), bring in a grants professional; this skill covers private foundation research.
