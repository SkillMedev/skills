...
Follow the steps in order. The niche grid must come before the statement, and the statement before the portfolio audit, because each step is the raw material for the next.

### Step 1: Collect the inputs

Gather these from the freelancer. If an answer is a guess, label it a guess and move on.

1. Last 10-15 paid projects: client type, problem solved, fee, and how enjoyable/repeatable it was.
2. The 2-3 projects with the best results the freelancer can prove (metrics, testimonials, before/after).
3. Industries the freelancer has genuine exposure to (worked in, sold to, or has a network in).
4. Current income and rate (Maya: $6,000/month at $75/hr).

### Step 2: Run the riches-in-niches grid (vertical x problem x outcome)

Build a three-column grid and generate candidate niches as combinations:

- **Vertical**: a buyer group with money and a way to reach them (DTC food and beverage brands, B2B SaaS founders, boutique law firms). "Small businesses" is not a vertical.
...
Use this formula, one sentence, no jargon:
...
Positioning without proof is a claim; proof without positioning is noise. Audit the portfolio:
...
- Minimum viable niche: at least 2 provable past results in it, or commit to one anchor project to create them.
- Vertical size sanity check: the freelancer should be able to name 50+ reachable target companies; if they cannot, the niche is too narrow or access is missing.
- Commit window: hold the niche for at least 6 months before judging it. Positioning compounds through referrals and repeat recognition; switching quarterly resets the clock to zero.
- Off-niche work during transition: capped at 50 percent of revenue, trending down each quarter. Bills must be paid, but every off-niche project should be invisible in public positioning.

## Deliverable

A one-page positioning document containing: the scored niche grid, the final positioning statement, the three test results (referral, exclusion, premium), and the portfolio action list (promote / demote / fill). This document is the input to productized-service-designer.

## Do NOT

- Do not niche by craft alone ("I do logos"). Craft niches commoditize; vertical-x-problem niches command premiums because the buyer hears their own situation.
...
The finished positioning passes when: the statement fits the formula in one sentence; it names a vertical a stranger could list companies in; the outcome is in buyer language; at least 2 portfolio pieces prove it; and it survives all three tests. If any check fails, return to the grid.

## Next in the pack

Take the positioning statement into productized-service-designer to turn the outcome into fixed-scope, fixed-price offers, then retainer-pricing-calculator to price them against real capacity. For reaching the 50+ target companies with outreach, use cold-email-craft.
