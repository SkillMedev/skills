...
Follow the steps in order. Research must precede hypotheses (hypotheses come from signals, not imagination), and hypotheses must precede the question plan (each question exists to confirm or kill a hypothesis).
...
### Step 3: Build the hypothesis stack

Write two to three hypotheses as falsifiable statements: "They are probably struggling with X because Y." Each must be specific enough to be wrong. "They have pain" is not a hypothesis. "Their ops team is manually reconciling data because they outgrew their current tool six months ago" is.

Every hypothesis gets at least one question in Step 4 designed to confirm or kill it.
...
- Pain (Identify pain / Pain): what hurts, in their words.
- Impact (Metrics / Impact): the cost of the status quo in hours, dollars, or risk.
...
Define exactly two to three outcomes the rep must leave with: a qualified or disqualified verdict, a specific pain confirmed or killed, and an agreed next step with a date on the calendar. A call that ends without a dated next step did not go well, regardless of how friendly it felt.

### Step 6: Fill and deliver the prep sheet

Populate the template below and hand it to the rep as the single pre-call artifact.

## Talk-time and in-call discipline

Bake these into the prep sheet notes:
...
RESEARCH SIGNALS (top 2 only)
  1. [FILL: signal + source]
  2. [FILL: signal + source]

HYPOTHESIS STACK (falsifiable)
  H1: They are probably struggling with [FILL] because [FILL].
  H2: [FILL]
  H3 (optional): [FILL]

QUESTION PLAN (5-7; * = protect if short on time)
  Situation:  [FILL]
  Problem:  * [FILL: tied to H1]
  Problem:    [FILL: tied to H2]
  Impact:   * [FILL: quantify cost of status quo]
  Impact:     [FILL]
  Process:    "Walk me through how you'd evaluate and decide on something like this."
  Timeline:   "Is there a date or event this needs to be solved before?"

QUALIFICATION COVERAGE (mark after call)
  Pain [ ]  Impact/Metrics [ ]  Decision process [ ]  Critical event [ ]
  Still unknown for call 2: [FILL: economic buyer / criteria / champion / competition]

CALL GOALS (2-3)
  1. Qualify or disqualify against [FILL: criterion]
  2. Confirm or kill H1
  3. Book next step: [FILL: specific step] by [FILL: date]

TALK-TIME TARGET: prospect 60%+ / rep under 40%
```

## Deliverable

Produce a completed discovery prep sheet containing the two research signals, a two-to-three hypothesis stack, a five-to-seven question plan with the two protected questions flagged and MEDDICC/SPICED coverage marked, and two to three call goals including a dated next step.
...
Before the prep sheet ships: every question maps to a hypothesis or a qualification element; the two protected questions are marked; decision-process and critical-event questions are present verbatim; the next-step goal names a concrete step and date; research took 20 minutes or less; anything guessed is labeled a guess.
