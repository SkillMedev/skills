...
Follow the steps in order. Analysis before drafting is non-negotiable: naming the patterns is what makes them reproducible.
...
Rhythm is the strongest fingerprint. Before anything qualitative, count across the samples:

- Mean sentence length in words.
- Distribution: percentage of sentences under 10 words, percentage over 25 words. An author who writes 20% short punchy sentences reads completely differently from one at 2%, even at the same mean.
- Fragments per 100 sentences (deliberate fragments are a signature, not an error).
- Paragraph shape: typical sentences per paragraph, and whether paragraphs open with the point or build to it.

### Step 3: Capture register, signature constructions, and stance
...
Complete the template below and show it to the user before drafting. This is the checkpoint where a wrong read costs one message instead of a full rewrite.
...
### Step 6: Audit before delivering

Compare draft to profile, line by line:

- Sentence mean within ±3 words of the profile; short/long percentages within 10 points.
- No word the author would not use; punctuation habits matched.
- Opens and closes the way the author does.
- Blind test: read one sample and one passage of the draft back to back. If which is which is detectable by feel, revise before delivering.
...
RHYTHM
  Mean sentence length:        [FILL] words
  Short sentences (<10 words): [FILL]%
  Long sentences (>25 words):  [FILL]%
  Fragments per 100 sentences: [FILL]
  Paragraph shape:             [FILL: e.g. 2-3 sentences, opens with the point]

REGISTER
  Vocabulary:    [FILL: plain/ornate, contractions, jargon, profanity]
  Punctuation:   [FILL: dashes/semicolons/ellipses/Oxford comma habits]
...
GAPS / GUESSES: [FILL: situations the samples don't cover]
```

## Register contrast example

Bad (ghostwriter's neutral default): "There are several important factors to consider when evaluating this approach, and it is worth examining each one carefully."

Good (matching a blunt, short-sentence author): "Three things matter here. Everything else is noise."

Both are competent. Only one is the author.

## Deliverable

Produce two artifacts: the completed voice profile (so the user can confirm or correct the read) and the drafted piece written to that profile, with a short note flagging any passages where voice was guessed because the samples did not cover that situation.
...
A draft ships only when: the sentence-length numbers match the profile within the audit tolerances; all five signature constructions could plausibly appear (and at least a couple do); the blind back-to-back read is inconclusive; and every guessed-voice passage is flagged for the user.
